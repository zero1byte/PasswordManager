...
struct Page{
    ...
    virtual auto write() -> void = 0;// assing 0 menas pure virtual function wich means it must be implemented in derived classes
    virtual auto read() -> void = 0;
    virtual auto bytes_len() -> std::size_t = 0;
    ...
    virtual auto balance_page_insert(std::streampos seek_parent, IndexPage<TYPES()>& parent, std::int32_t child_pos) -> void = 0;
    virtual auto balance_page_remove(std::streampos seek_parent, IndexPage<TYPES()>& parent, std::int32_t child_pos) -> void = 0;                                           
